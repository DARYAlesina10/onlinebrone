...
const foodRaw=<?php echo json_encode(array_values($foodItems), JSON_UNESCAPED_UNICODE); ?>; const foodFallback=[{name:'Сет «Стандарт»',price:5490,img:'',desc:'Пепперони, 4 сыра, ассорти фри, цезарь, фрукты'},{name:'Сет «Пицца»',price:4290,img:'',desc:'Пепперони, 4 сыра, мясная, чикен чиз, кордон блю'}]; const setDesc={'Сет «Стандарт»':'Пепперони, 4 сыра, ассорти фри, цезарь, фрукты','Сет «Пицца»':'Пепперони, 4 сыра, мясная, чикен чиз, кордон блю','Сет «Оптимальный»':'Пепперони, 4 сыра, ассорти фри, цезарь, фрукты, шашлычки','Сет «Фри»':'Фри, дольки, наггетсы, пельмени, корндоги, соусы','Сет «Максимальный»':'Сырная тарелка, шашлычки, оливье, цезарь, брускетты','Сет «Фуршет»':'Сырная тарелка, шашлычки, оливье, цезарь, брускетты, роллы'}; let food=(foodRaw||[]).filter(x=>(x.name||'').toLowerCase().includes('сет')).map(x=>({name:x.name,price:x.price,img:x.img,desc:setDesc[x.name]||''})); if(!food.length) food=foodFallback;
...
document.getElementById('backToForm').onclick=()=>{document.querySelector('.vr-main').style.display='block';setTimeout(()=>{document.querySelector('.vr-main').style.opacity='1';},20);document.getElementById('finalPanel').style.display='none';document.getElementById('vrSummary').style.display='none';setTimeout(()=>{document.getElementById('vrSummary').style.display='block';},20);}
