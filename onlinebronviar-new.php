...
const foodRaw=<?php echo json_encode(array_values($foodItems), JSON_UNESCAPED_UNICODE); ?>; const setDesc={'Сет «Стандарт»':'Пепперони, 4 сыра, ассорти фри, цезарь, фрукты','Сет «Пицца»':'Пепперони, 4 сыра, мясная, чикен чиз, кордон блю','Сет «Оптимальный»':'Пепперони, 4 сыра, ассорти фри, цезарь, фрукты, шашлычки','Сет «Фри»':'Фри, дольки, наггетсы, пельмени, корндоги, соусы','Сет «Максимальный»':'Сырная тарелка, шашлычки, оливье, цезарь, брускетты','Сет «Фуршет»':'Сырная тарелка, шашлычки, оливье, цезарь, брускетты, роллы'}; const food=(foodRaw||[]).filter(x=>(x.name||'').toLowerCase().includes('сет')).map(x=>({name:x.name,price:x.price,img:x.img,desc:setDesc[x.name]||''}));
...
document.getElementById('findArena').onclick=()=>{
  ...
  fetch(url).then(r=>r.text()).then(txt=>{
    ...
    if(!Array.isArray(arr)||!arr.length){let alt=''; const base=new Date(d); for(let dd=0;dd<7;dd++){const dt=new Date(base); dt.setDate(base.getDate()+dd); const ds=dt.toISOString().slice(0,10); ['10:00','12:00','14:00','16:00','18:00'].forEach(function(ti){const weekend=(dt.getDay()==0||dt.getDay()==6); const tIdx=tier(+document.getElementById('guests').value||8); const pk=state.package; const pr=pk?(weekend?pk.prices.we[tIdx]:pk.prices.wd[tIdx]):0; const st=weekend?'background:#3b2b2b':'background:#243b24'; alt += '<button class=\"ghost altTime\" data-date=\"'+ds+'\" data-time=\"'+ti+'\" style=\"margin:4px;'+st+'\">'+ds+' '+ti+' — '+pr+' ₽</button>';}); } box.innerHTML='<div class=\"card\">Нет свободных столов. Измените дату/время:</div>'+alt;return;}
